...
- ProductPulse app-owned mutation registry for editable action proposals that save real app-visible `ProductAction` data and never apply to Shopify.
...
- `AI_APP_MUTATIONS_ENABLED`: editable ProductPulse app mutation proposal/save switch.
...
- Authenticated app users can save ProductPulse app mutations only when app mutation and confirmation flags are enabled.
