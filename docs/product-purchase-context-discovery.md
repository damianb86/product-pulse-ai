...
QuickScan reads Shopify catalog and order data in `app/lib/product-pulse-quick-scan.server.js`.
...
- Because QuickScan reads all line items in scanned orders, it can infer basket composition by grouping sale events by `orderId`.

Deep diagnosis reads product-scoped Shopify evidence in `app/lib/product-pulse-diagnosis.server.js`.
...
Quantity per order line is available from Shopify `LineItem.quantity` in both QuickScan and deep diagnosis.
...
- QuickScan: inferred from complete sale events grouped by `orderId`.
- Deep diagnosis: read from `basketLineItems` attached to each product sale event.
...
- QuickScan and deep diagnosis only analyze the configured lookback window.
