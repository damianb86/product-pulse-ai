...
const safeActionPayloadSchema = z.object({
  ...
  proposedText: z.string().trim().max(5000).optional(),
  ...
  descriptionOperation: z.enum(["prepend", "append", "replace"]).optional(),
  insertionPosition: z.enum(["prepend", "append", "replace"]).optional(),
...
function editablePayload(payload: z.infer<typeof safeActionPayloadSchema>): Record<string, unknown> {
  ...
  return {
    ...
    ...(payload.draftText !== undefined ? { draftText: payload.draftText } : {}),
    ...(payload.proposedText !== undefined ? { proposedText: payload.proposedText } : {}),
    ...
    ...(payload.targetField !== undefined ? { targetField: payload.targetField } : {}),
    ...(payload.shopifyField !== undefined ? { shopifyField: payload.shopifyField } : {}),
    ...(payload.descriptionOperation !== undefined ? { descriptionOperation: payload.descriptionOperation } : {}),
    ...(payload.insertionPosition !== undefined ? { insertionPosition: payload.insertionPosition } : {}),
