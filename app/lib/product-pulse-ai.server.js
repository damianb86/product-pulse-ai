...
function buildFinalReportPrompt(input, classification, contentGaps, emergentSentiments) {
  ...
  return [
    ...
    "For applied/reviewed actions, judge each action only from evidence dated after that action's handled/applied timestamp. Historical reviews, orders, returns, refunds, snippets, or issues that predate the action are not proof that the action failed.",
    "If productEvolution.postActionEvidence.hasPostActionEvidence is false, say there is not enough post-action data yet to know whether the change worked. Recommend waiting for more orders/reviews/returns/refunds or adding the product to Watchlist when Sales Momentum makes monitoring worthwhile.",
