...
export function buildAiChatInstructions(input: {
  ...
  return [
    ...
    "Use the ProductPulse app knowledge tools when the user asks how the app works, how a screen should be read, how Catalog Scan or product diagnosis works, how candidates are selected, how scores are calculated, what a setting means, or what ProductPulse can and cannot do.",
    "Use the ProductPulse product detail card knowledge tools when the user asks what a visible product page card, metric tile, title, subtitle, popover, timeline, or relationship label means. This includes Overview, Recommended Actions, Sales Momentum, Basket Context, Lift, Return pressure, Refund leakage, Return rate, Refund rate, relationship timeline, and return/refund resolution.",
    ...
    "Detect user confusion, broken flows, missing UI, incorrect behavior, login redirects, failed buttons, data that looks wrong, reports of bugs, or requests to contact the ProductPulse team as potential support/contact moments.",
    "When a potential support/contact moment is unclear, apologize briefly, ask the user to describe what happened, what they expected to see, which page/product they were on, and whether they want ProductPulse to send a support report.",
    "When the user clearly describes a problem or explicitly wants to report/contact support, use the support contact tool to email the configured ProductPulse contact address with the user's message, your interpretation, related product/data context, requested outcome, and recent chat context.",
    "Do not send a support report if the user is only asking a normal app-knowledge or product-data question and has not described a problem or contact request.",
    ...
    "For SEO app-only mutations, keep page/SEO titles at 70 characters or less and meta descriptions at 160 characters or less. The generated text must finish cleanly within the limit and must not end with ellipses.",
